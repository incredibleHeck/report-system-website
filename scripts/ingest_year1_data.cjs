...
const targetFiles = [
  // 1A
  { filename: '2025 2026 - 1A TERM 1.csv', classCode: 'YEAR-1A', className: 'YEAR 1A', termCode: 'T1', termId: '2025-2026-T1', termName: 'Term 1' },
  { filename: '2025 2026 - 1A TERM 2.csv', classCode: 'YEAR-1A', className: 'YEAR 1A', termCode: 'T2', termId: '2025-2026-T2', termName: 'Term 2' },
  { filename: '2025 2026 - 1A TERM 3.csv', classCode: 'YEAR-1A', className: 'YEAR 1A', termCode: 'T3', termId: '2025-2026-T3', termName: 'Term 3' },
  // 1B
  { filename: '2025 2026 - 1B TERM 1.csv', classCode: 'YEAR-1B', className: 'YEAR 1B', termCode: 'T1', termId: '2025-2026-T1', termName: 'Term 1' },
  { filename: '2025 2026 - 1B TERM 2.csv', classCode: 'YEAR-1B', className: 'YEAR 1B', termCode: 'T2', termId: '2025-2026-T2', termName: 'Term 2' },
  { filename: '2025 2026 - 1B TERM 3.csv', classCode: 'YEAR-1B', className: 'YEAR 1B', termCode: 'T3', termId: '2025-2026-T3', termName: 'Term 3' },
];

async function run() {
  console.log('🚀 Starting Year 1A and 1B Historical Data Ingestion...');
  ...
  for (const enr of allEnrollments) {
    if (enr.classId === '2025-2026-YEAR-1A' || enr.classId === '2025-2026-YEAR-1B') {
  ...
  year1Students.sort((a, b) => (a.classId + a.index).localeCompare(b.classId + b.index));
  console.log(`📋 Found ${year1Students.length} registered Year 1 students in Firestore.`);
  ...
  for (const tFile of targetFiles) {
    ...
    const classStudents = year1Students.filter((s) => s.classId === classId);
  ...
  console.log(`✔ Ingested ${generatedScores.length} assessment scores and ${generatedSummaries.length} report summaries into Firestore for Year 1A & 1B.`);

  // Write static fallback TypeScript file src/data/year1HistoricalData.ts
  const outTs = `import type { AssessmentScore, ReportSummary } from "../types";\n\n` +
    `export const HISTORICAL_YEAR1_SCORES: AssessmentScore[] = ` + JSON.stringify(generatedScores, null, 2) + ` as any;\n\n` +
    `export const HISTORICAL_YEAR1_SUMMARIES: ReportSummary[] = ` + JSON.stringify(generatedSummaries, null, 2) + ` as any;\n`;

  fs.writeFileSync(path.join(__dirname, '..', 'src', 'data', 'year1HistoricalData.ts'), outTs);
  console.log(`✔ Written src/data/year1HistoricalData.ts successfully.`);
}
