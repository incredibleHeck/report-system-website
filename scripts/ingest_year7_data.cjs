...
class BatchChunker {
  constructor(firestoreDb, maxOps = 50) {
    this.db = firestoreDb;
    this.maxOps = maxOps;
...
const subjectsMap = [
  ...
  { code: 'CHEM', name: 'Chemistry', totalHeader: 'CHEM EOT 100', gradeHeader: 'CHEM GRADE', commentHeader: 'CHEM COMMENT', cwHeader: 'CHEM CW 20', mtHeader: 'CHEM MT 20', eot60Header: 'CHEM EOT 60' },
  { code: 'GEO', name: 'Geography', totalHeader: 'GEO EOT 100', gradeHeader: 'GEO GRADE', commentHeader: 'GEO COMMENT', cwHeader: 'GEO CW 20', mtHeader: 'GEO MT 20', eot60Header: 'GEO EOT 60' },
  { code: 'BIO', name: 'Biology', totalHeader: 'BIO EOT 100', gradeHeader: 'BIO GRADE', commentHeader: 'BIO COMMENT', cwHeader: 'BIO CW 20', altCw: 'BI0 CW 20', mtHeader: 'BIO MT 20', eot60Header: 'BIO EOT 60' },
  { code: 'PHY', name: 'Physics', totalHeader: 'PHY EOT 100', gradeHeader: 'PHY GRADE', commentHeader: 'PHY COMMENT', cwHeader: 'PHY CW 20', mtHeader: 'PHY MT 20', eot60Header: 'PHY EOT 60' },
  { code: 'HIST', name: 'History', totalHeader: 'HIST EOT 100', gradeHeader: 'HIST GRADE', commentHeader: 'HIST COMMENT', altTotal: 'HIST. EOT 100', altGrade: 'HIST. GRADE', altComment: 'HIST. COMMENT', cwHeader: 'HIST CW 20', altCw: 'HIST. CW 20', mtHeader: 'HIST MT 20', altMt: 'HIST. MT 20', eot60Header: 'HIST EOT 60', altEot60: 'HIST. EOT 60' },
  { code: 'LIT', name: 'Literature in English', totalHeader: 'LIT EOT 100', gradeHeader: 'LIT GRADE', commentHeader: 'LIT COMMENT', altTotal: 'LIT. EOT 100', altGrade: 'LIT. GRADE', altComment: 'LIT. COMMENT', cwHeader: 'LIT CW 20', altCw: 'LIT. CW 20', mtHeader: 'LIT MT 20', altMt: 'LIT. MT 20', eot60Header: 'LIT EOT 60', altEot60: 'LIT. EOT 60' },
  { code: 'PROJ', name: 'Project Work', totalHeader: 'PROJ EOT 100', gradeHeader: 'PROJ GRADE', commentHeader: 'PROJ COMMENT' },
];

const targetFiles = [
  { filename: '2025 2026 - 7 TERM 1.csv', classCode: 'YEAR-7', className: 'YEAR 7', termCode: 'T1', termId: '2025-2026-T1', termName: 'Term 1' },
  { filename: '2025 2026 - 7 TERM 2.csv', classCode: 'YEAR-7', className: 'YEAR 7', termCode: 'T2', termId: '2025-2026-T2', termName: 'Term 2' },
  { filename: '2025 2026 - 7 TERM 3.csv', classCode: 'YEAR-7', className: 'YEAR 7', termCode: 'T3', termId: '2025-2026-T3', termName: 'Term 3' },
];

async function run() {
  console.log('🚀 Starting Year 7 Historical Data Ingestion...');
  const chunker = new BatchChunker(db, 100);
  ...
  for (const enr of allEnrollments) {
    if (enr.classId === '2025-2026-YEAR-7') {
  ...
  year7Students.sort((a, b) => a.index.localeCompare(b.index));
  console.log(`📋 Found ${year7Students.length} registered Year 7 students in Firestore.`);
  ...
  for (const tFile of targetFiles) {
    ...
    for (let rIdx = 0; rIdx < dataRows.length; rIdx++) {
      ...
      if (studentName) {
        matchedStudent = year7Students.find((s) => nameMatch(s.name, studentName));
      }
      
      // Fallback matching by row position index (001, 002...) if name typo or header shift
      if (!matchedStudent) {
        const idxStr = String(rIdx + 1).padStart(3, '0');
        matchedStudent = year7Students.find((s) => s.index === idxStr);
      }
      ...
      const teacherName = idxTeacherName !== -1 && row[idxTeacherName] ? row[idxTeacherName] : 'CLASS TEACHER';
  ...
  console.log(`✔ Generated ${generatedScores.length} assessment scores and ${generatedSummaries.length} report summaries for Year 7.`);

  // Write static fallback TypeScript file src/data/year7HistoricalData.ts FIRST
  const outTs = `import type { AssessmentScore, ReportSummary } from "../types";\n\n` +
    `export const HISTORICAL_YEAR7_SCORES: AssessmentScore[] = ` + JSON.stringify(generatedScores, null, 2) + ` as any;\n\n` +
    `export const HISTORICAL_YEAR7_SUMMARIES: ReportSummary[] = ` + JSON.stringify(generatedSummaries, null, 2) + ` as any;\n`;

  fs.writeFileSync(path.join(__dirname, '..', 'src', 'data', 'year7HistoricalData.ts'), outTs);
  console.log(`✔ Written src/data/year7HistoricalData.ts successfully.`);

  // Queue Firestore batch writes
  for (const s of generatedScores) {
    const scoreRef = db.collection('sais_scores').doc(s.id);
    const stuMarkRef = db.collection('studentMarks').doc(s.id);
    await chunker.set(scoreRef, s, { merge: true });
    await chunker.set(stuMarkRef, s, { merge: true });
  }

  for (const sum of generatedSummaries) {
    const sumRef = db.collection('sais_reportSummaries').doc(sum.id);
    await chunker.set(sumRef, sum, { merge: true });
  }

  await chunker.commit();
  console.log(`✔ Ingested all records into Firestore for Year 7.`);
}
