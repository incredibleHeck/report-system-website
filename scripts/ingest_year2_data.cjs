...
const targetFiles = [
  // 2A
  { filename: '2025 2026 - 2A TERM 1.csv', classCode: 'YEAR-2A', className: 'YEAR 2A', termCode: 'T1', termId: '2025-2026-T1', termName: 'Term 1' },
  { filename: '2025 2026 - 2A TERM 2.csv', classCode: 'YEAR-2A', className: 'YEAR 2A', termCode: 'T2', termId: '2025-2026-T2', termName: 'Term 2' },
  { filename: '2025 2026 - 2A TERM 3.csv', classCode: 'YEAR-2A', className: 'YEAR 2A', termCode: 'T3', termId: '2025-2026-T3', termName: 'Term 3' },
  // 2B
  { filename: '2025 2026 - 2B TERM 1.csv', classCode: 'YEAR-2B', className: 'YEAR 2B', termCode: 'T1', termId: '2025-2026-T1', termName: 'Term 1' },
  { filename: '2025 2026 - 2B TERM 2.csv', classCode: 'YEAR-2B', className: 'YEAR 2B', termCode: 'T2', termId: '2025-2026-T2', termName: 'Term 2' },
  { filename: '2025 2026 - 2B TERM 3.csv', classCode: 'YEAR-2B', className: 'YEAR 2B', termCode: 'T3', termId: '2025-2026-T3', termName: 'Term 3' },
];

async function run() {
  console.log('🚀 Starting Year 2A and 2B Historical Data Ingestion...');
  ...
  for (const enr of allEnrollments) {
    if (enr.classId === '2025-2026-YEAR-2A' || enr.classId === '2025-2026-YEAR-2B') {
  ...
  year2Students.sort((a, b) => (a.classId + a.index).localeCompare(b.classId + b.index));
  console.log(`📋 Found ${year2Students.length} registered Year 2 students in Firestore.`);
  ...
  for (const tFile of targetFiles) {
    ...
    const classStudents = year2Students.filter((s) => s.classId === classId);
  ...
  console.log(`✔ Ingested ${generatedScores.length} assessment scores and ${generatedSummaries.length} report summaries into Firestore for Year 2A & 2B.`);

  // Write static fallback TypeScript file src/data/year2HistoricalData.ts
  const outTs = `import type { AssessmentScore, ReportSummary } from "../types";\n\n` +
    `export const HISTORICAL_YEAR2_SCORES: AssessmentScore[] = ` + JSON.stringify(generatedScores, null, 2) + ` as any;\n\n` +
    `export const HISTORICAL_YEAR2_SUMMARIES: ReportSummary[] = ` + JSON.stringify(generatedSummaries, null, 2) + ` as any;\n`;

  fs.writeFileSync(path.join(__dirname, '..', 'src', 'data', 'year2HistoricalData.ts'), outTs);
  console.log(`✔ Written src/data/year2HistoricalData.ts successfully.`);
}
