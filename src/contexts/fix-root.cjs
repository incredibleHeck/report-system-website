...
const rootProviderTs = `import React, { createContext, useContext, useState, useEffect, useRef, ReactNode, useMemo } from 'react';
import { 
...
import { deduplicateUsersByEmail, mintStudentKey, enrollmentFromClass, normalizeTermFields, gradeFromTotal, applyLegacyMigration, demoSubjectLines, buildDemoSnapshot } from './utils';
...
export function useActiveClass() {
  ...
  const classStudents = useMemo(() => {
    ...
    if (map.size === 0 && (activeClass.id === '2025-2026-YEAR-5A' || activeClass.id === 'demo-primary-class')) {
      lifelongStudents.forEach((l: any, idx: any) => {
...
fs.writeFileSync(path.join(__dirname, 'RootDatabaseProvider.tsx'), rootProviderTs);

const databaseContextTs = `// Re-export for backward compatibility
export { RootDatabaseProvider as DatabaseProvider, useDatabase, useActiveClass } from '../contexts/RootDatabaseProvider';
export { termKeyFromSettings, normalizeYearId, getStreamYearId, deduplicateUsersByEmail } from '../contexts/utils';
`;
fs.writeFileSync(path.resolve(__dirname, '../context/DatabaseContext.tsx'), databaseContextTs);
console.log('Fixed RootDatabaseProvider and DatabaseContext facade.');
